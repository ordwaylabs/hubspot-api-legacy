  ...
  describe '#add' do
    ...
    context 'static list' do
      it 'returns true if contacts have been added to the list' do
        contact = HubspotLegacy::Contact.all(count: 1).first
        mock(HubspotLegacy::Connection).post_json("/contacts/v1/lists/:list_id/add", {:params=>{:list_id=>4}, :body=>{:vids=>[contact.vid]}}) { { 'updated' => [contact.vid] } }
    ...
    context 'dynamic list' do
      it 'raises error if try to add a contact to a dynamic list' do
        contact = HubspotLegacy::Contact.new(example_contact_hash)
        expect { dynamic_list.add(contact) }.to raise_error(HubspotLegacy::RequestError)
      end
    end
  end
