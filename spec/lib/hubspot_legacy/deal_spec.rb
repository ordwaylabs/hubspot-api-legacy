describe HubspotLegacy::Deal do
  ...
  describe '.find_by_company' do
    ...
    let(:company) { HubspotLegacy::Company.create!('Test Company') }
    let(:deal) { HubspotLegacy::Deal.create!(portal_id, [company.vid], [vid], { amount: amount }) }
  ...
  describe '#destroy!' do
    ...
    it 'should remove from hubspot' do
      pending
      expect(HubspotLegacy::Deal.find(deal.deal_id)).to_not be_nil

      expect(deal.destroy!).to be_true
      expect(deal.destroyed?).to be_true

      expect(HubspotLegacy::Deal.find(deal.deal_id)).to be_nil
    end
  end
